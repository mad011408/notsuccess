"""
NEXUS AI Agent - NVIDIA Provider
...
class NvidiaProvider(BaseLLMProvider):
    """
    NVIDIA API Provider
    OpenAI-compatible endpoint for NVIDIA models
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(api_key or NVIDIA_API_KEY, **kwargs)
        self.base_url = base_url or NVIDIA_API_BASE_URL
    ...
    async def initialize(self) -> None:
        ...
        logger.info(f"NVIDIA Provider initialized with base URL: {self.base_url}")
    ...
    async def generate(
        ...
        **kwargs
        ...
        """Generate response using NVIDIA API (OpenAI-compatible)"""
        ...
        # NVIDIA API has lower max_tokens limit - cap at 4096
        ...
        if system_message:
            ...
            logger.info(f"[NVIDIA #{self._response_counter}] System prompt attached (length: {len(system_message)} chars)")
        else:
            logger.warning(f"[NVIDIA #{self._response_counter}] NO SYSTEM PROMPT!")
    ...
    async def generate_stream(
        ...
        **kwargs
        ...
        if system_message:
            payload["messages"] = [{"role": "system", "content": system_message}] + chat_messages
            logger.info(f"[NVIDIA STREAM #{self._response_counter}] System prompt attached (length: {len(system_message)} chars)")
        else:
            logger.warning(f"[NVIDIA STREAM #{self._response_counter}] NO SYSTEM PROMPT!")
    ...
    async def generate_with_tools(
        ...
        **kwargs
        ...
        if system_message:
            payload["messages"] = [{"role": "system", "content": system_message}] + chat_messages
            logger.info(f"[NVIDIA TOOLS #{self._response_counter}] System prompt attached (length: {len(system_message)} chars)")
        else:
            logger.warning(f"[NVIDIA TOOLS #{self._response_counter}] NO SYSTEM PROMPT!")
        ...
        try:
            ...
            logger.error(f"NVIDIA generate_with_tools error: {e}")
            raise
    ...
    async def close(self) -> None:
        ...
        logger.info("NVIDIA Provider closed")
